...
<meta charset="UTF-8" />
<title>예매 페이지</title>
<input type="hidden" value="<%= request.getParameter("senddate") %>"  id="day"/>
<link rel="stylesheet"
				...
				<div class="col " style="text-align: center;">
					...
					<div class="row">
						...
						<div class="col-4 border " id="selectSeat"
							...
							<h4>
								<b>"</b><span><b><%= m.getUserid() %></b></span><b>"</b> 회원님께서
							</h4>
									...
									<div class="modal-dialog" style="margin-top: 10%;">
										<div class="modal-content">
											...
											<div class="modal-body" id="modal"
												...
												<img id="select" src="/baseball/resources/images/ticketing/mid_view.png"
													width="100%">
											</div>
											<script>
											$('#Move15').on('click', function() {
												$('#select').attr("src","/baseball/resources/images/seatView/1base.jpg");				
												});
											
											$('#Move12').on('click', function() {
												$('#select').attr("src","/baseball/resources/images/seatView/right.jpg");				
											});
											
											$('#Move6').on('click', function() {
												$('#select').attr("src","/baseball/resources/images/seatView/left.jpg");				
											});
											
											$('#Move1').on('click', function() {
												$('#select').attr("src","/baseball/resources/images/seatView/catcher.jpg").
												css('height', '345px');				
											});
...
<script>
	...
	$('#Move18').on('click', function() {
		...
		document.getElementById('SeatCheck0').innerHTML = "로케트배터리 외야파티덱";
		if (isWeekend == true) {
			price += 120000;
		} else {
			price += 96000;
	...
	$('#payBtn').on('click', function() {
		...
		IMP.request_pay({
			...
			// stadium으로 슥삭
			buyer_name : '인천 SK 행복드림구장',
			buyer_postcode : <%=m.getUserno()%>,
			...
			if (rsp.success) {
				$.ajax({
					url : "/baseball/InsertT.do", //cross-domain error가 발생하지 않도록 동일한 도메인으로 전송
					type : 'POST',
					dataType : 'json',
					data : {
						// 서블릿에서 받을 값들 적으세요
				...
				*/
				...
				location.href = "http://192.168.20.132:8088/baseball/index.jsp?item=ticket&pay_method="+rsp.pay_method
				+ "&nick="+rsp.buyer_name + "&add="+rsp.buyer_addr
